...
<!--              <xsl:when test="/xml/infos/LIB_INF_MINI != 'null' and /xml/infos/LIB_INF_MINI != ''">-->
              <xsl:when test="not(/xml/infos/LIB_INF_MINI)">
...
<BR/>
       <form name="{$prefForm}MonDossierWeb" action="{$baseActionURL}?action=calExamAffiche&amp;etp={/xml/infos/etp}&amp;vet={/xml/infos/vet}" method="post">
